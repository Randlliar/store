...
export class CategoryFilterComponent implements OnInit {
  ...
  @Input() categoriesWithTypes: CategoryWithTypeType | null = null;
  ...
  get title(): string {
    if (this.categoriesWithTypes) {
      return this.categoriesWithTypes.name;
    } else if (this.type) {
      if (this.type === 'height') {
        return 'Высота';
  ...
  ngOnInit(): void {
    this.activatedRoute.queryParams.subscribe(params => {
      ...
      if (this.type) {
        ...
        if (this.categoriesWithTypes && this.categoriesWithTypes.types
          && this.categoriesWithTypes.types.length > 0 &&
          this.categoriesWithTypes.types.some(type  => this.activeParams.types.find(item => type.url === item))) {
          this.open = true;
        }
      }
    });
